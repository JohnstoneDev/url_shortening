	...
	return (
		<article className='flex flex-col gap-1.5'>
			...
			<div className='flex flex-col'>
				{
					contents.map(c => <span key={contents.indexOf(c)}>{c}</span>)
				}
			</div>
		</article>
	)
...
const Links = () => {
	return (
		<section className='bg-very-dark-violet text-slate-100 flex desktop:grid desktop:grid-cols-2 mobile:flex-col max-mobile:flex-col justify-center items-center p-6'>
